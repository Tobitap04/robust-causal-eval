...
class Evaluation:
    ...
    def __init__(self, llm_service: LLMService, llm: str, nq: int,
        ...
        Args:
            ...
            sample_path (str): Path to sample of the  Webis-CausalQA-22 dataset.
        ...
        self.latex = latex  # Format checked in get_cl_args()

    def run(self) -> None:
        """
        Handles the evaluation process by sampling questions, generating responses, and computing metrics.
        Raises:
        ...
        for idx, question in enumerate(sampled_questions, 0):
            ...
            try:
                ...
                if question_text is None or answer_text is None:
                    ...
                    continue
                # Create reference response without perturbations
                reference_response = processing_func(question_text, self.preprocessing, self.inprocessing,
                                                     self.postprocessing, dataset, self.llm_service, self.temperature)
                # Create responses for each perturbation level
                for perturbation in self.perturbation_levels:
                    # print(f"\r\033[KEvaluation in progress: Question {idx}/{num_questions}: \"{question_text[
                    # :60]}...\" | Perturbation: {perturbation} | Status: Generating results...", end="", flush=True)
                    try:
                        perturbed_question = question.get(f'question_{perturbation}_perturb')
                        hypothesis_response = processing_func(perturbed_question, self.preprocessing, self.inprocessing,
                                                              self.postprocessing, dataset, self.llm_service,
                                                              self.temperature)
                        # print(f"\nQuestion:\n {perturbed_question}")
                        # print(f"Response:\n {hypothesis_response}")
                        # print(f"Reference:\n {reference_response}")
                        ...
                        for metric in self.metrics:
                            try:
                                score = compute_metric(hypothesis_response, reference_response, answer_text,
                                                       perturbed_question, metric)
                                results[metric][perturbation].append(score)
                                # print(f"Metric {metric}: {score}") print(f"\rEvaluation in progress: Question {
                                # idx}/{num_questions}: \"{question_text[:60]}...\" | Perturbation: {perturbation} |
                                # Metric: {metric} | Status: Score calculated: {score:.4f}", end="", flush=True)
                            except Exception as e:
                                print()
                                logging.error(
                                    f"Evaluation: Error while computing metric {metric} of question {idx}: {e}")
                    except Exception as e:
                        print()
        ...
        # Calculate average results for each metric and perturbation level
